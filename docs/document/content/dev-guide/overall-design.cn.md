...
在选择数据库分片中间件方案前，对业界主流的数据库代理/分片工具进行了技术调研。调研重点围绕以下维度展开：

- **自定义外部路由**：是否支持用户自定义路由策略，特别是外部服务参与路由决策
- **高可用**：代理层和后端数据库的高可用保障机制
- **跨分片聚合去重**：跨分片 SQL 的聚合（SUM/AVG/COUNT/GROUP BY）和去重（DISTINCT）能力
- **SQL 多路由（双写）**：同一条 SQL 同时路由到多个数据节点的能力（如影子库双写、广播表同步）
...
| 调研维度 | 能力评估 |
|---------|---------|
| **自定义外部路由** | 支持通过 `catlet` 扩展自定义路由逻辑，但扩展机制较为简单，需要编写自定义类并配置在 `rule.xml` 中。不支持运行时动态加载，需要重启生效。MyCat 2.0 引入了 Prototype 概念，路由扩展性有所提升但社区活跃度低 |
| **高可用** | 支持 MySQL 主从心跳检测（`<heartbeat>` 配置），可在主库宕机时自动切换到备库。但 MyCat 自身的高可用需要借助外部工具（如 HAProxy + Keepalived）实现，不具备内置的集群协调能力 |
| **跨分片聚合去重** | 支持基本的跨分片聚合（SUM、COUNT、MAX、MIN），但对 `AVG` 的处理较为粗糙（早期版本直接求各分片平均值的平均值，存在精度问题）。`DISTINCT` 支持有限，复杂子查询场景下容易出现结果不正确 |
| **SQL 多路由（双写）** | 支持全局表（类似广播表），自动将 DML 操作路由到所有分片节点执行。但不支持影子库级别的双写能力，也不支持按规则选择性双写 |

**技术特点**：
- 基于拦截转发模型，在 MySQL 协议层直接代理
- 配置通过 `schema.xml`、`rule.xml`、`server.xml` 三大 XML 文件管理
- 支持 ER 分片（父子表关联分片）
- 社区活跃度逐年下降，MyCat 2.0 开发进度缓慢

#### 2.3.2 ShardingSphere

[Apache ShardingSphere](https://shardingsphere.apache.org/) 是 Apache 顶级项目，定位为 "Database Plus" 分布式数据库增强生态。

| 调研维度 | 能力评估 |
|---------|---------|
| **自定义外部路由** | 提供完善的 SPI 扩展机制，用户可实现 `ShardingAlgorithm` 接口自定义分片路由算法，支持 Standard（单键）、Complex（多键）、Hint（外部指定）三种策略模式。Hint 机制允许通过外部服务设置路由目标，运行时动态生效无需重启。此外，通过 DistSQL 可动态管理和切换分片规则 |
| **高可用** | 三层高可用设计：（1）系统级状态机 `OK → READ_ONLY → UNAVAILABLE` 自动降级与恢复；（2）Proxy 连接级熔断 `OK → CIRCUIT_BREAK`；（3）集群模式下多 Proxy 实例通过 ZooKeeper/Etcd 实现配置同步、节点发现和分布式锁。支持后端数据源探活自动触发状态转换 |
| **跨分片聚合去重** | 完善的跨分片结果归并引擎：排序归并使用优先级队列（多路归并排序），`AVG` 改写为 `SUM + COUNT` 在归并阶段重新计算保证精度，`GROUP BY` 支持流式分组归并，`DISTINCT` 基于排序的流式去重。支持装饰器链组合多种归并策略 |
| **SQL 多路由（双写）** | 支持多种多路由场景：（1）广播表将 DML 自动路由到所有数据节点；（2）影子库功能支持按 SQL 注释或列值判断，将流量同时路由到正式库和影子库；（3）DDL 语句通过 `TableBroadcastRouteEngine` 自动广播到所有分片节点 |

**技术特点**：
- 微内核 + SPI 可插拔架构，扩展性极强
- 双模接入：Proxy（独立代理）和 JDBC（嵌入式驱动）
- ANTLR4 多方言 SQL 解析，支持 11 种数据库方言
- Apache Calcite 联邦查询引擎
- 活跃的 Apache 开源社区，持续迭代

#### 2.3.3 Vitess

[Vitess](https://vitess.io/) 是由 Google 开源并捐赠给 CNCF 的数据库集群系统，最初为 YouTube 提供 MySQL 水平扩展能力，是 Kubernetes 原生的数据库分片方案。

| 调研维度 | 能力评估 |
|---------|---------|
| **自定义外部路由** | Vitess 通过 VSchema（Vitess Schema）定义路由规则，支持 `vindexes`（Virtual Index）机制来定义分片键到分片的映射。用户可实现自定义 Vindex 接口扩展路由策略，但扩展需要 Go 语言编写并编译到 VTGate 中，门槛较高。不支持运行时热加载自定义路由 |
| **高可用** | HA 能力突出：VTTablet + VTOrc 可自动检测主库故障并执行主从切换（PRS/ERS），内置拓扑服务发现，支持跨区域部署与灾难恢复。VTGate 无状态设计，可通过 K8s 水平扩展。但整体架构组件众多（VTGate/VTTablet/VTOrc/vtctld/Topology Service），运维复杂度高 |
| **跨分片聚合去重** | VTGate 的 SQL 执行引擎支持跨分片的 `ORDER BY`、`GROUP BY`、聚合函数和 `DISTINCT`。通过 scatter-gather 模式下发子查询到各分片，在 VTGate 层面进行结果归并。但对复杂子查询和多层嵌套查询的支持不如单体数据库完善 |
| **SQL 多路由（双写）** | 支持 `REPLICA` 类型的只读路由，但不原生支持影子库双写。通过 Materialize 和 VReplication 功能可以实现跨 keyspace 的数据同步，间接达到类似效果。Multi-shard DML 可以在一个事务中写入多个分片 |

**技术特点**：
- Go 语言开发，Kubernetes 原生部署
- 组件化架构：VTGate（代理层）、VTTablet（分片管理）、VTOrc（编排）
- 强大的在线 DDL 能力（`gh-ost` / `pt-osc` 集成）
- VReplication 支持数据流和物化视图
- CNCF 毕业项目，在云原生场景下生态完善

#### 2.3.4 ProxySQL

[ProxySQL](https://proxysql.com/) 是高性能的 MySQL 协议代理，专注于 MySQL/Percona/MariaDB 的读写分离和查询路由。

| 调研维度 | 能力评估 |
|---------|---------|
| **自定义外部路由** | 通过查询规则（Query Rules）和正则匹配实现 SQL 路由，支持基于 SQL 文本、Schema、用户名、客户端地址等多维度的路由规则。规则通过 Admin 接口动态管理，运行时生效。但不支持基于分片键值的计算路由，本质上是查询级别的路由而非数据分片路由 |
| **高可用** | 内置后端健康检查和连接池管理，支持自动故障检测和故障转移。可与 MHA、Orchestrator 等集成实现主从切换。ProxySQL 自身通过 Cluster 功能实现多实例配置同步。但 HA 能力聚焦于连接管理和查询路由，不涉及分片级别的高可用 |
| **跨分片聚合去重** | 不支持。ProxySQL 定位为查询路由代理而非分片代理，不具备将 SQL 拆分到多个分片执行后归并结果的能力。所有 SQL 路由到单一后端组执行 |
| **SQL 多路由（双写）** | 支持通过 Query Rules 实现 SQL 镜像（Mirroring），将查询同时发送到主后端组和镜像后端组。但镜像功能主要用于性能测试，不保证镜像端的写入一致性。不支持分片级别的广播写入 |

**技术特点**：
- C++ 开发，极致性能（单实例可处理数十万 QPS）
- 强大的连接池和多路复用能力
- 运行时通过 Admin 接口动态配置
- 专注于 MySQL 协议，不支持其他数据库
...
| 维度 | MyCat | ShardingSphere | Vitess | ProxySQL |
|------|-------|----------------|--------|----------|
| **自定义外部路由** | ⭐⭐ catlet 扩展，重启生效 | ⭐⭐⭐⭐⭐ SPI + Hint + DistSQL 动态管理 | ⭐⭐⭐ Vindex 自定义，需 Go 编译 | ⭐⭐ Query Rules 正则匹配 |
| **高可用** | ⭐⭐ 心跳检测，自身 HA 依赖外部 | ⭐⭐⭐⭐ 三层 HA + 集群模式 | ⭐⭐⭐⭐⭐ VTOrc 自动主从切换 | ⭐⭐⭐ 健康检查 + 故障转移 |
| **跨分片聚合去重** | ⭐⭐ 基本聚合，DISTINCT 支持有限 | ⭐⭐⭐⭐⭐ 完善的多路归并引擎 | ⭐⭐⭐⭐ scatter-gather 归并 | ❌ 不支持分片 |
| **SQL 多路由（双写）** | ⭐⭐ 全局表广播 | ⭐⭐⭐⭐⭐ 广播表 + 影子库 + DDL 广播 | ⭐⭐⭐ VReplication 间接实现 | ⭐⭐ SQL 镜像（仅查询） |
| **多数据库支持** | MySQL | MySQL/PG/Oracle/SQLServer 等 11 种 | MySQL | MySQL |
| **接入方式** | Proxy | Proxy + JDBC 双模 | Proxy (VTGate) | Proxy |
| **开发语言** | Java | Java | Go | C++ |
| **社区活跃度** | 低（趋于停滞） | 高（Apache 顶级项目） | 高（CNCF 毕业项目） | 中（专注 MySQL） |
| **扩展机制** | XML 配置 | SPI 热插拔 | Go Plugin | Admin API |

**结论**：

1. **ShardingSphere 在扩展性和功能完整性方面表现最优**：SPI 机制使其在自定义路由、跨分片归并、多路由场景上均具备开箱即用的完善支持，且支持运行时动态管理
2. **Vitess 在 HA 和云原生部署方面领先**：VTOrc 自动编排能力和 K8s 原生支持使其在高可用场景下表现突出，但 Go 语言生态与现有 Java 技术栈不匹配
3. **MyCat 功能基本满足但社区活力不足**：核心功能可用，但缺少持续演进，复杂场景（如 DISTINCT、子查询）存在已知缺陷
4. **ProxySQL 定位不同**：专注于查询路由和连接池优化，不具备数据分片能力，适合纯读写分离场景

综合评估，ShardingSphere 在 **自定义路由扩展性**、**跨分片聚合精确性**、**多路由（双写）完整性**、**多数据库兼容性** 以及 **Java 技术栈一致性** 方面均为最优选择，作为本系统的分片代理核心框架。
